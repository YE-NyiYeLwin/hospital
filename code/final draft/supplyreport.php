	...
	<?php
		if (!empty($_REQUEST['term'])) {
			...
			while ($row = mysqli_fetch_array($r_query))
			{
		      ...
		      $suppliername=$connect->query("SELECT companyname FROM dbsupplier where ID='$supplierid'")->fetch_object()->name;
				...
				";
				$sql2 = "SELECT * FROM dbsupplydetail where supplyID='$ID'";
		...
		{
			$sql = "SELECT * FROM dbsupply ORDER BY ID";
			$r_query = mysqli_query($connect,$sql); 
			...
			for ($i=0; $i < $count; $i++) { 
		      ...
		      $totalprice=$row['totalprice'];
				...
				$staffname=$connect->query("SELECT name FROM dbstaff where ID='$staffid'")->fetch_object()->name;
		      $suppliername=$connect->query("SELECT companyname FROM dbsupplier where ID='$supplierid'")->fetch_object()->companyname;
